...
export const Skills: React.FC = () => {
  ...
  // Helper to filter skills based on search + selected category
          ...
          <AnimatePresence mode="popLayout">
            {filteredSkillsData.map((categoryGroup) => (
              <motion.div
                ...
                className="glass-panel p-6 rounded-2xl border border-white/5 relative group hover:border-white/10 transition-colors duration-300"
              >
                {/* Category Header */}
                ...
                <div className="flex flex-wrap gap-2.5">
                  {categoryGroup.skills.map((skill, sIdx) => (
                    <span
                      key={sIdx}
                      className="text-xs font-medium text-gray-300 bg-dark-card/50 hover:bg-gold-500/5 hover:text-gold-400 hover:border-gold-500/30 px-3 py-1.5 rounded-xl border border-white/5 transition-all duration-300"
                    >
                      {skill}
                    </span>
                  ))}
